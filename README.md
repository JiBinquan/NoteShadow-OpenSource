...
> **项目状态**：当前配置为 `0.5.0` 发布候选版，仍在设备验证中。公开发行版尚未确定；仓库内部分语音模型的再分发条款仍待核实。请先阅读[许可与分发说明](#许可与分发)。
...
项目**自有代码和文档**采用 [Apache License 2.0](LICENSE)；本项目横幅、应用图标和演示截图采用 [CC BY 4.0](ASSETS_LICENSE.md)。第三方代码、原生库、模型和测试音频按各自条款处理，详见[第三方说明](THIRD_PARTY_NOTICES.md)。公开源码将另行制作不含模型文件和旧 Git 历史的快照；在确认模型再分发授权前，不发布含模型的 APK。
